    ...
    try {
      ...
      if (isLive === true) {
        ...
        try {
          ...
          // Return all collected real API events
          if (allApiEvents.length > 0) {
            console.log(`✅ Returning ${allApiEvents.length} REAL API-Sports live events`);
          ...
          if (reqSportId) {
            ...
            if (filteredEvents.length > 0) {
              ...
              // POLICY: ZERO fake/adapted data - return empty if no real API data
              console.log(`⚠️ NO REAL API DATA FOR SPORT ID ${reqSportId} - Returning empty results per strict API-only policy`);
              return res.json([]);
            }
          } else {
