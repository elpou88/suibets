...
export function BettingButton({ name, odds, onClick, isActive = false }: BettingButtonProps) {
  return (
    <div className="flex-1 min-w-[85px] mx-1 flex justify-center">
      <Button
        variant="outline"
        className={`w-full py-3 h-auto min-h-[70px] ${
          isActive 
      ...
      >
        <div className="flex flex-col items-center justify-center w-full">
          ...
          <span className="text-xl font-bold text-cyan-300 w-full text-center whitespace-nowrap">{odds.toFixed(2)}</span>
        </div>
      </Button>
    </div>
  );
