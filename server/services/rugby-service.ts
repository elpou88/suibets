...
/**
 * Service for rugby API interactions
 * Documentation: https://api-sports.io/documentation/rugby/v1
 */
export class RugbyService {
  private apiKey: string;
  private apiUrl: string = 'https://v1.rugby.api-sports.io/games';
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn('[RugbyService] No API key provided. API requests will likely fail.');
    }
  }
  
  /**
   * Get live rugby events
   */
  async getLiveEvents(): Promise<SportEvent[]> {
    const cacheKey = 'rugby_live_events';
    
    // Check cache first
    const cachedData = this.getFromCache(cacheKey);
    if (cachedData) {
      console.log('[RugbyService] Using cached live events data');
      return cachedData;
    }
    
    try {
      console.log('[RugbyService] Fetching live rugby events');
      ...
      if (!response.data || !response.data.response) {
        console.error('[RugbyService] Invalid response format for live events', response.data);
        return [];
      }
      ...
      console.log(`[RugbyService] Fetched ${events.length} live rugby events`);
      return events;
    } catch (error) {
      console.error('[RugbyService] Error fetching live rugby events:', error);
      return [];
    }
  }
  ...
  async getUpcomingEvents(limit: number = 20): Promise<SportEvent[]> {
    const cacheKey = `rugby_upcoming_events_${limit}`;
    
    // Check cache first
    const cachedData = this.getFromCache(cacheKey);
    if (cachedData) {
      console.log('[RugbyService] Using cached upcoming events data');
      return cachedData;
    }
    
    try {
      console.log(`[RugbyService] Fetching upcoming rugby events (limit: ${limit})`);
      ...
      if (!response.data || !response.data.response) {
        console.error('[RugbyService] Invalid response format for upcoming events', response.data);
        return [];
      }
      ...
      console.log(`[RugbyService] Fetched ${limitedEvents.length} upcoming rugby events`);
      return limitedEvents;
    } catch (error) {
      console.error('[RugbyService] Error fetching upcoming rugby events:', error);
      return [];
    }
  }
  
  /**
   * Get a specific rugby event by ID
   */
  async getEventById(eventId: string): Promise<SportEvent | null> {
    try {
      console.log(`[RugbyService] Fetching rugby event with ID: ${eventId}`);
      ...
      if (!response.data || !response.data.response || !response.data.response.length) {
        console.error('[RugbyService] Event not found or invalid response format', response.data);
        return null;
      }
      ...
      console.error(`[RugbyService] Error fetching rugby event with ID ${eventId}:`, error);
      return null;
    }
  }
  
  /**
   * Transform rugby API response data to our SportEvent format
   */
  private transformEvents(responseData: any[], isLive: boolean): SportEvent[] {
    if (!Array.isArray(responseData)) {
      console.error('[RugbyService] Response data is not an array:', responseData);
      return [];
    }
    ...
    return responseData.map((event) => {
      try {
        ...
        return {
          id: game.id?.toString() || `rugby-${Date.now()}`,
          sportId: 6, // Rugby has ID 6
          leagueId: league.id?.toString() || '',
          leagueName: league.name || 'Rugby League',
          homeTeam,
          awayTeam,
