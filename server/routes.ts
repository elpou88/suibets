    ...
    try {
      ...
      if (reqSportId) {
        ...
        if (realEvents && realEvents.length > 0) {
          ...
          const strictEnrichedRealEvents = filterEventsBySportId(enrichedRealEvents, reqSportId);
          return res.json(strictEnrichedRealEvents);
        } else {
    ...
    try {
      ...
      try {
        ...
        if (liveEvents && liveEvents.length > 0) {
          ...
          const strictLive = filterEventsBySportId(liveEvents, sportId);
          return res.json(strictLive);
        }
      } catch (directError) {
